...
const CartRecent: FC<CartRecentProps> = ({ size, content, time, icon }) => {
  ...
  return (
    <div style={{ height: `${size}rem` }} className={`flex flex-col justify-between rounded-xl shadow-sm p-4 bg-white bg-opacity-90 cursor-pointer hover:bg-white/90 hover:shadow-lg`}>
      <div className={icon ? '' : 'hidden'}>
        <MessagesSquare className="w-4 text-black/70" />
      </div>
      <h2 className="font-semibold text-sm text-black/90">{content?.titletMsg?.text}</h2>
